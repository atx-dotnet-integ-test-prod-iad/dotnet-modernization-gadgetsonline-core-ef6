...
{
    // ---------------------------------------------------------------------------
    // EF6 PostgreSQL (Npgsql) provider configuration
    // Registers the Npgsql ADO.NET provider and connection factory with EF6 so
    // that the existing base("name=GadgetsOnlineEntities") constructor continues
    // to resolve the connection string from the application configuration file.
    // ---------------------------------------------------------------------------
    ...
    public class GadgetsOnlineEntities : DbContext
    {
        ...
        // -----------------------------------------------------------------------
        // SaveChanges overrides — normalise DateTime values to UTC so that Npgsql
        // does not throw on timestamps that lack an explicit DateTimeKind.
        // -----------------------------------------------------------------------
        ...
        {
            // -------------------------------------------------------------------
            // Table and column mappings — dbo (SQL Server) → gadgetsonline_dbo
            // (PostgreSQL).  All target names are lowercase to match the
            // PostgreSQL objects created by the schema migration tooling.
            // -------------------------------------------------------------------
            ...
            // -------------------------------------------------------------------
            // Relationship configurations — preserved exactly from the original
            // -------------------------------------------------------------------
