...
namespace GadgetsOnline.Models
{
    ...
    // Registers Npgsql as the ADO.NET provider and connection factory so that
    // Entity Framework 6 routes all database operations through Npgsql instead
    // of the default SQL Server provider.
    // -------------------------------------------------------------------------
    ...
    [System.Data.Entity.DbConfigurationType(typeof(GadgetsOnlineEntitiesPostgreSqlConfiguration))]
    public class GadgetsOnlineEntities : DbContext
    {
        ...
        // -----------------------------------------------------------------
        // DateTime UTC fix: PostgreSQL requires timestamps to carry UTC kind.
        // FixDateTimeKinds() normalises any Local/Unspecified DateTime values
        // to UTC before they reach the Npgsql driver, preventing
        // "Cannot write DateTime with Kind=Local" runtime errors.
        // -----------------------------------------------------------------
        ...
        {
            // -----------------------------------------------------------------
            // Schema: all tables live in the 'gadgetsonline_dbo' PostgreSQL
            // schema, which mirrors the original SQL Server 'dbo' schema.
            // Individual ToTable() calls below override this for every entity
            // so the mapping is explicit and immune to convention changes.
            // -----------------------------------------------------------------
            ...
            // -----------------------------------------------------------------
            // Relationships — preserved exactly from the original configuration
            // -----------------------------------------------------------------
            modelBuilder.Entity<Category>()
