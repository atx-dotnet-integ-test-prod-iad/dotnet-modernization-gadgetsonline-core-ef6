...
{
    // ---------------------------------------------------------------------------
    // PostgreSQL DbConfiguration for EF6 + Npgsql
    // ---------------------------------------------------------------------------
    ...
    {
        // ------------------------------------------------------------------
        // Static helper: allows ASP.NET Core (net8) callers to register the
        // PostgreSQL connection string once at startup so the parameterless
        // constructor can still be used by repositories / services that rely
        // on the EF6 "name=..." convention (which does not work with
        // appsettings.json in a net8 host).
        //
        // Usage in Program.cs / Startup.cs:
        //   GadgetsOnlineEntities.ConnectionString =
        //       builder.Configuration.GetConnectionString("GadgetsOnlineEntities");
        // ------------------------------------------------------------------
        public static string ConnectionString { get; set; }

        // Default constructor — uses the connection string registered above,
        // falling back to the legacy "name=" config-file lookup.
        public GadgetsOnlineEntities()
            : base(ConnectionString ?? "name=GadgetsOnlineEntities")
        {
        ...
        // ------------------------------------------------------------------
        // SaveChanges overrides — ensure all DateTime values are stored as
        // UTC so that Npgsql does not throw "timestamp with time zone"
        // conversion errors.
        // ------------------------------------------------------------------
