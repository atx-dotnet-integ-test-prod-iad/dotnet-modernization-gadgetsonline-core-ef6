...
{
    // EF6 PostgreSQL provider configuration — registers Npgsql as the ADO.NET
    // provider for EntityFramework 6 and sets NpgsqlConnectionFactory as the
    // default connection factory so EF6 resolves connections correctly.
    ...
    public class GadgetsOnlineEntities : DbContext
    {
        ...
        // ---------------------------------------------------------------------------
        // DateTime UTC fix for PostgreSQL — Npgsql requires DateTimeKind.Utc for
        // timestamp columns.  Both synchronous and async paths are covered so that
        // every Add/Modified entity has its DateTime values normalised before the
        // underlying ADO.NET command is issued.
        // ---------------------------------------------------------------------------
