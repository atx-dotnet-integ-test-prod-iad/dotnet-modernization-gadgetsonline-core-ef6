...
{
    // EF6 PostgreSQL DbConfiguration — registers Npgsql as the ADO.NET provider
    // and sets the default connection factory so EF6 routes all database calls
    // through Npgsql instead of the SQL Server provider.
    ...
    public class GadgetsOnlineEntities : DbContext
    {
        ...
        // ---------------------------------------------------------------------------
        // DateTime compatibility: Npgsql requires DateTime values stored in the
        // database to carry DateTimeKind.Utc.  These SaveChanges overrides normalise
        // every DateTime on Added/Modified entities before the SQL is sent so that
        // existing code that creates DateTime values without an explicit Kind still
        ...
        {
            // -----------------------------------------------------------------------
            // Table & column mappings — target schema: gadgetsonline_dbo (PostgreSQL)
            // -----------------------------------------------------------------------
            ...
            // -----------------------------------------------------------------------
            // Relationships (preserved exactly from original)
            // -----------------------------------------------------------------------
